...
library(tidyverse)
library(knitr)
# d = read.table("/home/neele/XPRAG/liveHITS/newfinal.csv",header=T,sep="\t",quote="")
d = read.table("../data/final.csv",header=T,sep="\t",quote="")
colnames(d) = c("TGrepID","EntireSentence","NumBNBForms","PropGoodLocation","ResponseModal","ResponseFreqGoodLocation","ResponsePropGoodLocation","SentenceOverlap")
...
as.character(d$ResponseModal)
f <- function(modalresponse) {
  finalbnb = ""
  bnb1 = strsplit(modalresponse, split=' | ', fixed=TRUE)[[1]][1]
  bnb2 = strsplit(modalresponse, split=' | ', fixed=TRUE)[[1]][2]
  res1 = str_match(bnb1, "or(.*?)+ but not both")
  res2 = str_match(bnb2, "or(.*?)+ but not both")
  print("substring")
  print(as.character(res1))
  print(as.character(res2))
  if (nchar(as.character(res1))[1] > nchar(as.character(res2))[1]){
   finalbnb = bnb2
  }
  else {
    finalbnb = bnb1
...
want = which(d$MoreThanOneModalResponse == TRUE)
want
d$NearestBNBSentence = NA
for (i in want){
  print("index")
  print(i)
  bnb = f(as.character(d$ResponseModal[i]))
  d$NearestBNBSentence[i] = as.character(bnb)
  d$FinalBNBSentence[i] = as.character(bnb)
}
d$FinalBNBSentence = as.factor(as.character(d$FinalBNBSentence))
d$NearestBNBSentence = as.factor(as.character(d$NearestBNBSentence))
```
...
kable(dsub[dsub$NumBNBForms > 4,c("ResponseModal","ResponseFreqGoodLocation","ResponsePropGoodLocation","NearestBNBSentence","FinalBNBSentence")])
```
