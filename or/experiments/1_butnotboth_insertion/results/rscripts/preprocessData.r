...
source("../results/rscripts/helpers.R")
...
#all results are read into one dataframe (d) with 11952 rows
nrow(d)
...
nrow(d)
names(d)
length(unique(d$tgrep.id)) #1244 cases left
...
# histogram of number of data points per tgrep ID after removing the "but not both" cases right after "or"
sorted = data.frame(table(d$tgrep.id))
table(sorted$Freq)
d[d$tgrep.id == sorted[sorted$Freq == 1,]$Var1,c("tgrep.id","entire.sentence")]
d[d$tgrep.id == sorted[sorted$Freq == 1,]$Var1,]$entire.sentence
