...
if (n_args > 1 || n_args %% 2 != 0) {
  ...
  d = read.csv(args[1], quote="")
  d$speaker_cond = args[2]
  
  n_files = n_args/2
  if (n_files > 1) {
    for (i in 2:n_files) {
      max_workerid = max(d$anon_workerid)
      d2 = read.csv(args[i*2 - 1])
      d2$speaker_cond = args[i*2]
      d2$anon_workerid = d2$anon_workerid + max(d$anon_workerid) + 1
      d = rbind(d, d2)
    }
  }
  
  write.csv(d, file="merged_workers.csv", row.names = FALSE, quote = FALSE)
