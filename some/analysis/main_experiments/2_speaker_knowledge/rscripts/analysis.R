...
sentences = read_csv("unique_sentences.csv") %>% 
  rename(tgrep_id=Item)

df = df %>% 
  left_join(sentences,by=c("tgrep_id"))
...
write.csv(d, file = "means_speakerknowledge.csv")
...
# get examples with low, medium, and high ratings
agr = sentences %>%
...
lowmeans = agr[agr$Mean <.3,c("Mean","Sentence")] %>% 
  arrange(Mean)
lowmeans

highmeans = agr[agr$Mean >.75,c("Mean","Sentence")] %>% 
  arrange(Mean)
highmeans
