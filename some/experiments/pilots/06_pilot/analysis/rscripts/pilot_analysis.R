...
df <-df[!(df$workerid %in% c("3", "9", "13")),]

#EXCLUDE WORKERS --> ASSESMENT - "Confused"
#df <-df[!(df$workerid %in% c("0","6","7","12")),]

#EXCLUDE WORKERS  --> ATTENTION CHECK -  1 person got 3 wrong, 6ppl got 2 wrong, 3ppl got 1 wrong, 4 got all right 
df <-df[!(df$workerid %in% c("13")),]

#EXCLUDE WORKERS  --> VARIABILITY - 2 SD away from mean in at least one item
df <-df[!(df$workerid %in% c("0","5")),]
